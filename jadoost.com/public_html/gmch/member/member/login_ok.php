...
<?php	
	...
	if(password_verify($password, $hash_pw)) //만약 password변수와 hash_pw변수가 같다면 세션값을 저장하고 알림창을 띄운후 main.php파일로 넘어갑니다.
	{
		...
		$_SESSION['userpw'] = $member["pw"];
		 $state_1 = mq("update member set login_state='1' where id='".$member["id"]."'");
		echo "<script> location.href='http://jadoost.com/gmch';</script>";
		...
		echo "<script> location.href='http://jadoost.com/gmch/member/index.php';</script>";
	}
}
?>