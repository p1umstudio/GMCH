    ...
    <table class="list-table">
         ...
         if(isset($_GET['page'])){
          $page = $_GET['page'];
              ...
              $sql = mq("select * from report_board");
              ...
              $sql2 = mq("select * from report_board order by report_b_no desc limit $start_num, $list");  
              while($report_board = $sql2->fetch_array()){
              $title=$report_board["title"]; 
                if(strlen($title)>1000)//문자열 값이 x이상일경우
                { 
                 $title = str_replace($report_board['title'],mb_substr($report_board['title'],0,3,'utf-8'),'...',$report_board['title']);
                }
    ...
    <tr>
      ...
      <td id="table_left" width="500" colspan="2" ><?php 
        ...
        if($report_board['lock_post']=="1"){ ?>
          <a href='board/ck_read.php?report_b_no=<?php echo $report_board["report_b_no"];?>'><?php echo $title; ?><span class="re_ct">[<?php echo $rep_count; ?>]<?php echo $img; ?></span><? echo $lockimg;
            }else{    
        ...
        <a href='board/read.php?report_b_no=<?php echo $report_board["report_b_no"]; ?>'><?php echo $title;?><span class="re_ct">[<?php echo $rep_count; ?>]<?php echo $img; ?></span><? }?>
        </a></td>
    <tr>
      <td id="table_left_line">
          <div id="id_mm-dd_hit">
            <?php echo $report_board['id']?> &#124;
            <?php $origDate = $report_board['date'];
