...
<body>
    ...
    <input type="button" class="img-button" 
            onclick="location.href='https://jadoost.com/gmch/4board/4board.php'">
        <p id="id">자유게시판</p>
        </header>
...
<div id="board_area">
    ...
    <table class="list-table">
         ...
         if(isset($_GET['page'])){
          $page = $_GET['page'];
              ...
              $sql2 = mq("select * from board order by b_no desc limit $start_num, $list");  
              while($board = $sql2->fetch_array()){
              $title=$board["title"]; 
                if(strlen($title)>1000)//문자열 값이 x이상일경우
                { 
                 $title = str_replace($board['title'],mb_substr($board['title'],0,3,'utf-8'),'...',$board['title']);
                }
   ...
   <tbody id="red">
    <tr>
      <td id="no_color" width="70" rowspan="2"><?php echo $board['b_no']; ?></td>
      <td id="table_left" width="500" colspan="2" ><?php 
        $lockimg = "<img src='../image/lock.png' alt='lock' title='lock' with='20' height='20' />";
        
        if($board['lock_post']=="1"){ ?>
          <a href='board/ck_read.php?b_no=<?php echo $board["b_no"];?>'><?php echo $title; ?><span class="re_ct">[<?php echo $rep_count; ?>]<?php echo $img; ?></span><? echo $lockimg;
            }else{    
        ...
        <a href='board/read.php?b_no=<?php echo $board["b_no"]; ?>'><?php echo $title;?><span class="re_ct">[<?php echo $rep_count; ?>]<?php echo $img; ?></span><? }?>
        </a></td>
    <tr>
      <td id="table_left_line">
          <div id="id_mm-dd_hit">
            <?php echo $board['id']?> &#124;
            <?php $origDate = $board['date'];
